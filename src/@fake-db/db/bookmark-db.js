...
const bookmarkDB = {
	bookmark: [
		...
		},
		{
			...
			description: '',
			archive: false,
			image: 'assets/images/bookmark/beach.jpeg',
		...
		},
		{
			...
			checklist: [
				...
				},
				{
					id: '3',
					checked: true,
					text: 'Keybookmark 1 - Brad Green  Jules Kremer'
				},
				{
					id: '4',
					checked: false,
		...
		},
		{
			...
			description: '',
			archive: false,
			image: 'assets/images/bookmark/road-trip.jpeg',
...
mock.onGet('/api/bookmark-app/bookmark').reply(config => {
	return [200, bookmarkDB.bookmark];
});

mock.onGet('/api/bookmark-app/labels').reply(config => {
	return [200, bookmarkDB.labels];
});

mock.onPost('/api/bookmark-app/create-bookmark').reply(request => {
	const data = JSON.parse(request.data);
	const newBookmark = {
		...data.bookmark,
		id: FuseUtils.generateGUID()
	};
	bookmarkDB.bookmark = [newBookmark, ...bookmarkDB.bookmark];
	return [200, newBookmark];
});

mock.onPost('/api/bookmark-app/update-bookmark').reply(request => {
	const { bookmark } = JSON.parse(request.data);

	bookmarkDB.bookmark = bookmarkDB.bookmark.map(_bookmark => {
		if (bookmark.id === _bookmark.id) {
			return bookmark;
		}
		return _bookmark;
	});
...
mock.onPost('/api/bookmark-app/update-labels').reply(request => {
	const data = JSON.parse(request.data);

	bookmarkDB.labels = data.labels;

	return [200, bookmarkDB.labels];
});

mock.onPost('/api/bookmark-app/remove-bookmark').reply(request => {
	const data = JSON.parse(request.data);

	bookmarkDB.bookmark = bookmarkDB.bookmark.filter(bookmark => data.bookmarkId !== bookmark.id);

	return [200, data.bookmarkId];
});
